...
# OMR Grading Configuration
cam = input("Do you want to use camera? (y/n): ") # if y, then True else False
if cam.lower() == "y":
    cam=True
    path=None
else:
    cam=False
    path = input("Enter the path of video file: ")# If cam is false, enter the image path here

NO_q = int(input("Enter number of Questions to be graded. "))
NO_c = int(input("Enter number of Choices each question has? "))
ans_key = []
# [1,2,0,2,4]
for i in range(1, NO_q+1):
    ans_key.append(int(input("Enter answer key for Q"+str(i)+": "))-1)  # Example : For Q5 answer key will be 5
    print(ans_key)
########################################################################
webCamFeed = cam
pathImage = path
